...
"""
CURRENT SETTINGS QUITE GOOD, TRY TO REDUCE OVERFITTING?
...
def define_train_model(train_features, train_labels, epochs, batch_size, sequence_length, xval, yval):
    ...
    layers = [
        ...
        LSTM(hidden_units, kernel_regularizer=L2(0.15), recurrent_regularizer=L2(0.15)),#,
...
def main():
    ...
    """
    merged.drop(labels=["WS_ms_TMx","WS_ms_TMn", "Target_Depth", "TCDT"], axis=1, inplace=True)
    #na_merged = merged[merged.isna()]
    #na_merged.to_csv("mergedNaAnalysis")
    ...
    """
    features = merged.drop("DBTCDT", axis=1)
    ...
    # fit to train
    forest.fit(X_train, y_train.ravel())#_scaled, y_train)
